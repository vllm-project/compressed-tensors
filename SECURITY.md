...
Please report security issues privately using [the vulnerability submission form](https://github.com/vllm-project/compressed-tensors/security/advisories/new).
...
compressed-tensors handles model weight deserialization and quantization operations. Security considerations include:

- Safe deserialization of model weights from untrusted sources
- Protection against malicious quantization configurations
- Secure handling of tensor operations
...
For certain security issues of CRITICAL, HIGH, or MODERATE severity level, we may prenotify certain organizations or vendors that ship compressed-tensors. The purpose of this prenotification is to allow for a coordinated release of fixes for severe issues.
...
* Organizations and vendors who either ship or use compressed-tensors, are eligible to join the prenotification group if they meet at least one of the following qualifications
    * Substantial internal deployment leveraging the upstream compressed-tensors project.
    * Established internal security teams and comprehensive compliance measures.
    * Active and consistent contributions to the upstream compressed-tensors project.
