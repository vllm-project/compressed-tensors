...
def _run_job_on_device(job: Callable[[torch.device], Any], device: torch.device) -> Any:
    """Run a job with the worker thread's assigned CUDA device selected.

    CUDA current-device state is thread-local. Passing ``cuda:N`` to a job is
    not enough for kernels that validate pointers against the current device.
    Keep allocation and kernel launch in the same device context for the full
    job. Non-CUDA backends retain their existing behavior.
    """
