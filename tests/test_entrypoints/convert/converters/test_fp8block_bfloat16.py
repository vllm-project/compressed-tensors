...
def test_fp8_block_to_bfloat16_conversion():
    ...
    # Verify using helper
    verify_block_conversion(result, weight_fp8, weight_scale_inv, (128, 128))
...
def test_fp8_block_converter_process():
    ...
    tensors = {
        "model.layer0.mlp.up_proj.weight": torch.randn(256, 256, dtype=torch.float32).to(
            torch.float8_e4m3fn
        ),
        "model.layer0.mlp.up_proj.weight_scale_inv": torch.randn(
            num_row_blocks, num_col_blocks, dtype=torch.float32
        ),
        "model.layer1.mlp.down_proj.weight": torch.randn(256, 256, dtype=torch.float32).to(
            torch.float8_e4m3fn
        ),
    ...
    assert (
        "model.layer0.mlp.up_proj.weight" in tensors
    ), "weight should still exist"
    assert (
        "model.layer1.mlp.down_proj.weight" in tensors
    ), "weight should still exist"

    # Verify the conversion is correct using helper
    verify_block_conversion(
